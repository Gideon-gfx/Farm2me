...
export type EscrowStatus =
  ...
  // the buyer to share their delivery code.
  | "ARRIVED"
  | "DELIVERED"
  | "DISPUTED"
  | "RELEASED";
...
export interface User {
  id: string;
  // Null for a Google signup that hasn't linked a phone number yet (mobile
  // doesn't have a Google sign-in flow itself yet, but shares the same
  // backend accounts as web, so this can still show up here).
...
export interface Listing {
  ...
  // Smallest quantity (kg) a buyer may order — defaults to 1 (effectively no
  ...
  // registered address) wins over the farmer's saved profile location
  // everywhere a pickup point is used.
  locationLabel?: string | null;
  locationLat?: number | null;
  locationLng?: number | null;
